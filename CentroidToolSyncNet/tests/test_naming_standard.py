"""Unit tests for Centroid description namnstandard + gap-fill enrich."""

from __future__ import annotations

import unittest

from lib.centroid_parser import from_bridge_dict, parse_description
from lib.tool_templates import build_tool_json, enrich_tool_json, patch_tool_json


def _tool(description: str, **kwargs):
    row = {
        "tool_number": kwargs.get("tool_number", 2),
        "h_number": kwargs.get("h_number", 2),
        "d_number": kwargs.get("d_number", 4),
        "offset": 0,
        "diameter": kwargs.get("diameter", 0),
        "coolant": "OFF",
        "spindle": "CW",
        "speed": kwargs.get("speed", 3500),
        "description": description,
    }
    return from_bridge_dict(row)
